...
当前版本为 **v1.12**。这是仓库和离线安装包的唯一维护版本；本次把会话历史调整为
右侧主工作区，连接设置默认隐藏，并在顶部增加当前模型、消息数、本轮资料、字符预算
以及技能/MCP 状态摘要。发送新消息时只追加和更新本轮内容，不再清空并重画已有历史。
...
优先使用 `FilePromptAI-Win7-Full-v1.12.zip`。完整解压后运行
