# FilePrompt AI v1.18 开发、测试与发布维护说明

本文面向开发者和发布维护者。普通用户只需阅读 [README.md](README.md)，下载
FilePromptAI-Win7-Full-v1.18.zip、完整解压并双击 Start-FilePromptAI.exe。普通用户
...
~~~powershell
powershell -ExecutionPolicy Bypass -File .\build-offline-package.ps1 -Version 1.18
~~~

打包脚本生成发布目录、PACKAGE-CHECKSUMS-SHA256.txt 和
FilePromptAI-Win7-Full-v1.18.zip。只准备并检查发布目录、不生成 ZIP 时使用：

~~~powershell
powershell -ExecutionPolicy Bypass -File .\build-offline-package.ps1 -Version 1.18 -StageOnly
~~~
...
~~~powershell
powershell -ExecutionPolicy Bypass -File .\tests\RunAllSmokeTests.ps1 -Version 1.18
~~~
...
正式包应从可信的带注释 v1.18 Git 标签读取固定摘要：

~~~powershell
git show v1.18:src/RELEASE-SHA256.txt
~~~

本地已测试但尚未正式封存的候选包，从被 Git 忽略的本地测试收据读取身份：

~~~powershell
Get-Content .\tests\build-artifacts\release\ReleaseCandidate-v1.18.txt
~~~

可信候选提交中的 Git LFS 指针，其 oid sha256 是 ZIP 内容摘要：

~~~powershell
git show <可信候选提交>:exe/FilePromptAI-Win7-Full-v1.18.zip
~~~

对实际文件重新计算：

~~~powershell
(Get-FileHash ..\exe\FilePromptAI-Win7-Full-v1.18.zip -Algorithm SHA256).Hash
~~~
...
FilePromptAI-Win7-Full-v1.18.zip；该 ZIP 由根目录 .gitattributes 使用 Git LFS
跟踪，不放摘要副本或其他说明文件。
...
~~~powershell
powershell -ExecutionPolicy Bypass -File .\tests\RunAllSmokeTests.ps1 -Version 1.18 -WriteReleaseReceipt
~~~

只有全部测试和离线包验证成功后，脚本才会在被 Git 忽略的
tests\build-artifacts\release\ReleaseCandidate-v1.18.txt 写入结构版本 2 的本地
测试收据。收据绑定：
...
~~~powershell
powershell -ExecutionPolicy Bypass -File .\promote-release-candidate.ps1 -Version 1.18
~~~
...
~~~text
exe/FilePromptAI-Win7-Full-v1.18.zip
~~~
...
FilePromptAI-Win7-Full-v1.18.zip 的原始 ZIP。验收器在运行期间保持 ZIP 的只读
...
~~~powershell
powershell -ExecutionPolicy Bypass -File .\seal-release.ps1 -Version 1.18 -AcceptanceReportPath 'D:\Acceptance\FilePromptAI-Acceptance-....xml'
~~~
...
- 源码候选提交到安装包晋升提交恰好只修改单个 v1.18 ZIP；
- 索引为空，除即将生成的两份证据外工作树干净；
- exe 精确只含与收据一致的 ZIP；
- XML、同名校验文件、结构版本、总 PASS、退出码 0、v1.18 验收器和全部必需检查
  都有效；
...
带注释的 v1.18 Git 标签只能指向 Win7 验收封存提交。封存后不得再次运行任何会重建 ZIP
的命令。

### 5. 标签后复核

创建带注释的 v1.18 Git 标签后运行：

~~~powershell
powershell -ExecutionPolicy Bypass -File .\tests\VerifyTaggedRelease.ps1 -Version 1.18 -AcceptanceReportPath 'D:\Acceptance\FilePromptAI-Acceptance-....xml'
~~~
