...
$testExecutable = Join-Path $artifactRoot 'ToolLoopSmokeTest.exe'
...
$arguments = @(
    ...
    (Join-Path $testRoot 'ToolLoopSmokeTest.cs')
)

& $compiler $arguments
if ($LASTEXITCODE -ne 0) {
    throw "Tool loop test compilation failed with exit code $LASTEXITCODE."
}

& $testExecutable
if ($LASTEXITCODE -ne 0) {
    throw "Tool loop test failed with exit code $LASTEXITCODE."
}
