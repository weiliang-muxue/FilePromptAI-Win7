...
$testExecutable = Join-Path $artifactRoot 'ModelProfileSmokeTest.exe'
...
$arguments = @(
    ...
    (Join-Path $projectRoot 'src\ModelProfiles.cs'),
    (Join-Path $testRoot 'ModelProfileSmokeTest.cs')
)

& $compiler $arguments
if ($LASTEXITCODE -ne 0) {
    throw "Model profile smoke test compilation failed with exit code $LASTEXITCODE."
}

& $testExecutable
if ($LASTEXITCODE -ne 0) {
    throw "Model profile smoke test failed with exit code $LASTEXITCODE."
}
