...
$testExecutable = Join-Path $artifactRoot 'CodeWorkspaceToolProviderSmokeTest.exe'
...
$arguments = @(
    ...
    (Join-Path $testRoot 'CodeWorkspaceToolProviderSmokeTest.cs')
)

& $compiler $arguments
if ($LASTEXITCODE -ne 0) {
    throw "Code workspace tool provider compilation failed with exit code $LASTEXITCODE."
}

& $testExecutable
if ($LASTEXITCODE -ne 0) {
    throw "Code workspace tool provider test failed with exit code $LASTEXITCODE."
}
