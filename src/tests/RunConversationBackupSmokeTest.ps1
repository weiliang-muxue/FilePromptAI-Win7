...
$testExecutable = Join-Path $artifactRoot 'ConversationBackupSmokeTest.exe'
...
$compilerArguments = @(
    ...
    (Join-Path $testRoot 'ConversationBackupSmokeTest.cs')
)

& $compiler $compilerArguments
if ($LASTEXITCODE -ne 0) {
    throw "Conversation backup test compilation failed with exit code $LASTEXITCODE."
}

& $testExecutable $artifactRoot
if ($LASTEXITCODE -ne 0) {
    throw "Conversation backup test failed with exit code $LASTEXITCODE."
}
