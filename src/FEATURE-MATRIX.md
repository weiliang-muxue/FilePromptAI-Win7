...
本文档按常见桌面 AI 客户端能力核对 FilePrompt AI v1.17 源码。它描述产品能力和明确
边界，不替代测试报告、目标 Windows 7 验收报告或发布包 SHA-256 证明。只有 annotated
`v1.17` 标签中的正式摘要和证据、同一 ZIP 的成功测试 receipt、规定环境中的 Windows 7
PASS XML（验收时另附同名校验文件）全部通过封存与标签后复核时，该 ZIP 才是正式
发布包；否则均按候选处理。
发布证据链为 `C`（源码候选）→ `P`（单 ZIP 原字节晋升）→ `S`（两文件 PASS 证据封存）；
...
| 状态 | 含义 |
| --- | --- |
| 已实现 | v1.17 源码已有完整用户路径及对应实现或自动测试证据；这是实现状态，不单独表示已经正式发布。 |
| v1.17 补齐项 | 本轮为 v1.17 增加或加固；只有最终源码、离线包、自动回归和目标机验收全部一致并通过发布门禁时，才属于正式发布。 |
...
| Windows 路径解析单飞与超时恢复 | v1.17 补齐项 | `src/MainForm.cs` 的 `pendingPathResolution` 和 `AddFilesAsync` 对路径检查设置 15 秒等待上限；旧探测仍由 Windows 收尾时拒绝再起 worker，并保留失败路径供重试；`tests/UiStateSmokeTest.cs` 的 `TestPathResolutionSingleFlight` 覆盖阻塞、释放和恢复。 | 不能强制中止已进入操作系统的网络路径调用；单飞门禁用于避免重复操作累积后台任务。 |
...
| 包完整性与可验证发布证据 | v1.17 补齐项 | `C` 的 schema 2 receipt 绑定测试产物，并仅保留在 Git 忽略的 `src/tests/build-artifacts/release`；完整套件安全解压刚构建的 ZIP，核对精确文件集和全部清单哈希。`promote-release-candidate.ps1` 要求 `C` 已淘汰旧资产，把 receipt 绑定的同一字节 ZIP 写入 `exe/FilePromptAI-Win7-Full-v1.17.zip`，复核 `exe` 精确只含该 ZIP并再次运行安装旅程，任何失败都回滚单个 ZIP；`C`→`P` 只改变该路径，ZIP 由仓库根 `.gitattributes` 通过 Git LFS 跟踪。正式封存生成 `src/RELEASE-SHA256.txt` 和 `src/RELEASE-EVIDENCE.txt`，直接绑定 receipt、ZIP identity 与 Win7 报告。 | `P` 仍是候选，只有 Win7 PASS 报告实际存在并通过 `S` 的封存及标签后复核才可称正式发布；`exe` 不包含任何摘要副本、说明或其他文件。 |
...
7. 发布拓扑固定为 `C`→`P`→`S`：`C` 是 receipt 绑定的源码候选并已删除旧版交付资产；receipt 只保留在 Git 忽略的 `src/tests/build-artifacts/release`；晋升前后都复核 `exe` 精确只含 `FilePromptAI-Win7-Full-v1.17.zip`；`P` 是 `C` 的直接子提交且只改变这一 ZIP 路径，该 ZIP 由仓库根 `.gitattributes` 通过 Git LFS 跟踪；待取得匹配的 Win7 PASS XML 后，`S` 是 `P` 的直接子提交且只含 `src/RELEASE-SHA256.txt`、`src/RELEASE-EVIDENCE.txt` 两个文件，正式证据直接绑定 receipt、ZIP identity 和 Win7 报告，不依赖 `exe` 中任何附加文件。
...
`1920x1080@96 DPI` 是 v1.17 一键验收程序的固定真实环境门禁，不是客户端运行时的
