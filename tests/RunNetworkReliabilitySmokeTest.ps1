...
$testExecutable = Join-Path $artifactRoot 'NetworkReliabilitySmokeTest.exe'
...
$arguments = @(
    ...
    (Join-Path $testRoot 'NetworkReliabilitySmokeTest.cs')
)

& $compiler $arguments
if ($LASTEXITCODE -ne 0) {
    throw "Network reliability test compilation failed with exit code $LASTEXITCODE."
}

& $testExecutable ([IO.Path]::GetFullPath($ApplicationPath))
if ($LASTEXITCODE -ne 0) {
    throw "Network reliability test failed with exit code $LASTEXITCODE."
}
