...
$testExecutable = Join-Path $artifactRoot 'GenerationSettingsSmokeTest.exe'
...
$arguments = @(
    ...
    (Join-Path $testRoot 'GenerationSettingsSmokeTest.cs')
)

& $compiler $arguments
if ($LASTEXITCODE -ne 0) {
    throw "Generation settings test compilation failed with exit code $LASTEXITCODE."
}

& $testExecutable ([IO.Path]::GetFullPath($ApplicationPath))
if ($LASTEXITCODE -ne 0) {
    throw "Generation settings test failed with exit code $LASTEXITCODE."
}
