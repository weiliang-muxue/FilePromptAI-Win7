...
$testExecutable = Join-Path $artifactRoot 'ExtractorHardeningSmokeTest.exe'
...
$arguments = @(
    ...
    (Join-Path $testRoot 'ExtractorHardeningSmokeTest.cs')
)

& $compiler $arguments
if ($LASTEXITCODE -ne 0) {
    throw "Extractor hardening test compilation failed with exit code $LASTEXITCODE."
}

& $testExecutable ([IO.Path]::GetFullPath($ApplicationPath))
if ($LASTEXITCODE -ne 0) {
    throw "Extractor hardening test failed with exit code $LASTEXITCODE."
}
