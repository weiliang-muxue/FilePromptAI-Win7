...
$arguments = @(
    ...
    (Join-Path $testRoot 'UiStateSmokeTest.cs')
)

& $compiler $arguments
if ($LASTEXITCODE -ne 0) {
    throw "UI state test compilation failed with exit code $LASTEXITCODE."
}

& $testExecutable ([IO.Path]::GetFullPath($ApplicationPath))
if ($LASTEXITCODE -ne 0) {
    throw "UI state test failed with exit code $LASTEXITCODE."
}
